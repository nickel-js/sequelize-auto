...
AutoSequelize.prototype.run = function(callback) {
	...
	function generateText(err) {
		...
		async.each(_.keys(self.tables), function(table, _callback){
			...
			text[table] += spaces + "var currentModel = sequelize.define('" + tableName + "', {\n";
			...
			text[table] += ");\n";
			text[table] += spaces + "currentModel && (currentModel.associate = function(models){\n";
			_.each(fields, function(field, i){
				// Find foreign key
				var foreignKeyBase = self.foreignKeys[table] && self.foreignKeys[table][field] ? self.foreignKeys[table][field] : null;
				var foreignKey = (foreignKeyBase && foreignKeyBase.isForeignKey) ? foreignKeyBase : null;
				if (foreignKey) {
					var asName = foreignKey.source_column;
					if (asName.substring(asName.length - 2, asName.length) == 'Id') {
						asName = asName.substring(0, asName.length - 2);
					} else
					if (asName.substring(asName.length - 3, asName.length) == '_id') {
						asName = asName.substring(0, asName.length - 3);
					}
					text[table] += spaces + spaces + "currentModels.belongsTo(models['" + foreignKey.target_table + "'], {foreignKey: '" + foreignKey.source_column + "', as: '" + asName + "'});\n";
				}
			});
			text[table] += spaces + spaces + "console.log('associating');\n";
			text[table] += spaces + "});\n";
			text[table] += spaces + "return currentModel;\n";
			text[table] += "};\n";
			text[table] += "";
			_callback(null);
		}, function(){
